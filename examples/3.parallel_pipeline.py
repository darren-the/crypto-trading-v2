...
def partition_func(element, num_partitions):
    if element[-1] == 'worker0':
        return 0
    else:
        return 1
...
with beam.Pipeline(options=options) as p:
    worker0, worker1 = (p | beam.Create([0]) | beam.ParDo(FetchOCHL()) | beam.Partition(partition_func, 2))
    worker0 | 'worker0' >> beam.io.WriteToText('gs://crypto-trading-v2-dataflow/placeholder-pipeline-output-0', file_name_suffix='.txt')
    worker1 | 'worker1' >> beam.io.WriteToText('gs://crypto-trading-v2-dataflow/placeholder-pipeline-output-1', file_name_suffix='.txt')
